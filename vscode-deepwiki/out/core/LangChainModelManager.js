...
exports.LangChainModelManager = void 0;
...
class LangChainModelManager {
        ...
        try {
            ...
            switch (this.config.provider) {
                case 'openai':
                    ...
                    this.model = new openai_1.ChatOpenAI(openaiConfig);
                ...
                case 'azure':
                    ...
                    this.model = new azure_openai_1.AzureChatOpenAI({
    ...
    async generateContent(prompt, context) {
        ...
        try {
            ...
            const messages = [
                ...
                new messages_1.HumanMessage(fullPrompt)
            ];
     ...
     */
    async generateDeepResearchContent(query, config, conversationHistory = [], contextText = '', fileContent = '', documents, repositoryInfo) {
        this.ensureModelInitialized();
        try {
            const systemPrompt = this.buildDeepResearchSystemPrompt(config);
            let fullPrompt = this.buildDeepResearchPrompt(query, conversationHistory, contextText, fileContent);
            // 添加文档上下文
...
- 确保分析的准确性和实用性`;
     ...
     */
    buildDeepResearchSystemPrompt(config) {
     ...
     * 构建简单分析提示词
     */
    buildSimpleAnalysisPrompt(repositoryType, repositoryUrl, repositoryName, language) {
        return `<role>
...
- 使用markdown格式提高可读性
     ...
     * 构建首次迭代提示词
     */
    buildFirstIterationPrompt(repositoryType, repositoryUrl, repositoryName, language) {
        return `<role>
你是一位专业的代码分析专家，正在分析 ${repositoryType} 仓库: ${repositoryUrl} (${repositoryName})。
...
- 在相关时引用特定文件和代码部分
     ...
     * 构建中间迭代提示词
     */
    buildIntermediateIterationPrompt(repositoryType, repositoryUrl, repositoryName, language, iteration) {
        return `<role>
你是一位专业的代码分析专家，正在分析 ${repositoryType} 仓库: ${repositoryUrl} (${repositoryName})。
...
- 在相关时引用特定文件和代码部分
     ...
     * 构建最终迭代提示词
     */
    buildFinalIterationPrompt(repositoryType, repositoryUrl, repositoryName, language) {
        return `<role>
你是一位专业的代码分析专家，正在分析 ${repositoryType} 仓库: ${repositoryUrl} (${repositoryName})。
...
- 在适当时以可行的洞察或建议结束
     ...
     * 构建上下文信息
     */
    buildContextFromDocuments(documents, repositoryInfo) {
        if (!documents || documents.length === 0) {
            return '';
        }
        let context = '';
     ...
     * 按文件路径分组文档
     */
    groupDocumentsByPath(documents) {
        const grouped = {};
     ...
     */
    buildDeepResearchPrompt(query, conversationHistory, contextText, fileContent) {
        let prompt = '';
        // 添加对话历史
...
exports.LangChainModelManager = LangChainModelManager;
//# sourceMappingURL=LangChainModelManager.js.map