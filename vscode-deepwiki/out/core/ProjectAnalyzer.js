...
exports.ProjectAnalyzer = exports.FileType = void 0;
...
class ProjectAnalyzer {
    constructor(excludedDirs = [], excludedFiles = [], includedDirs = [], includedFiles = []) {
     ...
     */
    async analyzeProject(projectPath, progressCallback) {
        ...
        try {
            ...
            if (await this.fileExists(packageJsonPath)) {
                ...
                if (deps.react)
                    ...
                    architecture.framework = 'Fastify';
            }
            // 分析架构层级
     ...
     * 文件优先级排序
     */
    prioritizeFiles(files, projectPath) {
     ...
     * 计算文件重要性
     */
    calculateFileImportance(filePath, projectPath) {
        const fileName = path.basename(filePath);
        const relativePath = path.relative(projectPath, filePath);
        const extension = path.extname(filePath).toLowerCase();
        let importance = 0;
     ...
     * 分析文件元数据
     */
    async analyzeFileMetadata(filePath, content, fileType) {
        const fileName = path.basename(filePath);
        const metadata = {
     ...
     * 检测主要目录
     */
    detectMainDirectories(projectPath, files) {
     ...
     * 检测架构层级
     */
    async detectArchitectureLayers(projectPath) {
        const layers = [];
        // 常见的架构层级目录
        const layerPatterns = [
            { pattern: /^(src|source)$/i, type: 'presentation', name: 'Source Code' },
            { pattern: /^(components?|views?|pages?)$/i, type: 'presentation', name: 'Presentation Layer' },
            { pattern: /^(services?|business|logic)$/i, type: 'business', name: 'Business Logic' },
            { pattern: /^(data|models?|entities?)$/i, type: 'data', name: 'Data Layer' },
            { pattern: /^(utils?|helpers?|lib|libraries?)$/i, type: 'infrastructure', name: 'Infrastructure' },
            { pattern: /^(config|configurations?)$/i, type: 'infrastructure', name: 'Configuration' }
        ];
        try {
     ...
     * 构建项目结构树 - 增强版本
     */
    async buildProjectStructure(dirPath, analyzedFiles) {
     ...
     * 智能文件扫描 - 根据项目类型和重要性过滤
     */
    async smartScanFiles(projectPath, projectType) {
        const allFiles = await this.scanFiles(projectPath);
        // 根据项目类型调整扫描策略
