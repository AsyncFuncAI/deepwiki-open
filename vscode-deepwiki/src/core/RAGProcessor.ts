...
import { DocumentChunk, ProcessedFileInfo } from './DocumentProcessor';
...
export class OpenAIProvider implements AIProvider {
    ...
    async generateResponse(prompt: string, context: string[], config: any): Promise<string> {
        const messages = [
            {
                role: 'system',
...
export class QwenProvider implements AIProvider {
    ...
    constructor(apiKey: string, model: string = 'qwen-turbo', baseURL: string = 'https://dashscope.aliyuncs.com/compatible-mode/v1', workspaceId?: string) {
        this.apiKey = apiKey;
        this.model = model;
        this.baseURL = baseURL;
...
export class LocalAIProvider implements AIProvider {
    ...
    constructor(endpoint: string, model: string = 'local-model') {
...
export class RAGProcessor {
     ...
     * 检索相关文档
     */
    private async retrieveRelevantDocuments(query: RAGQuery): Promise<RetrievedDocument[]> {
        ...
        const filteredResults = results.documents.filter((doc: any) => ((doc as any).relevanceScore || 0) >= threshold);
        
        // 转换为 RetrievedDocument 格式
        return filteredResults.map((doc: Document) => ({
            id: doc.id,
            content: doc.content,
            similarity: (doc as any).relevanceScore || 0,
            metadata: {
                filePath: doc.path || '',
     ...
     * 准备上下文
     */
    private prepareContext(retrievedDocs: RetrievedDocument[], query: RAGQuery): string[] {
     ...
     * 计算置信度
     */
    private calculateConfidence(retrievedDocs: RetrievedDocument[], query: RAGQuery): number {
        if (retrievedDocs.length === 0) {
            return 0;
        }
    ...
    async testAIConnection(): Promise<boolean> {
        try {
            const testResponse = await this.aiProvider.generateResponse(
