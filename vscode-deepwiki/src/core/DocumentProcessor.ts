...
export class DocumentProcessor {
    ...
    private deepWikiConfig?: DeepWikiConfig;
    ...
    // 配置文件扩展名
    private readonly CONFIG_EXTENSIONS = new Set([
    ...
    private shouldExcludeDirectory(dirName: string, fullPath: string): boolean {
        ...
        if (this.config.includePatterns && this.config.includePatterns.length > 0) {
            return !this.config.includePatterns.some((pattern: string) => 
                this.matchPattern(relativePath, pattern) || this.matchPattern(dirName, pattern)
            );
        }
    ...
    private shouldExcludeFile(fileName: string, fullPath: string): boolean {
        ...
        if (this.config.includePatterns && this.config.includePatterns.length > 0) {
            return !this.config.includePatterns.some((pattern: string) => 
                this.matchPattern(relativePath, pattern) || this.matchPattern(fileName, pattern)
            );
        }
     ...
     * 处理单个文件
     */
    private async processFile(filePath: string): Promise<ProcessedFileInfo | null> {
        try {
     ...
     * 对单个文件进行分块
     */
    private chunkFile(file: ProcessedFileInfo): DocumentChunk[] {
        const chunks: DocumentChunk[] = [];
     ...
     * 按行分块代码文件
     */
    private chunkCodeFile(file: ProcessedFileInfo): DocumentChunk[] {
        const chunks: DocumentChunk[] = [];
     ...
     * 按段落分块文档文件
     */
    private chunkDocumentFile(file: ProcessedFileInfo): DocumentChunk[] {
        const chunks: DocumentChunk[] = [];
     ...
     * 按字符分块
     */
    private chunkByCharacters(file: ProcessedFileInfo): DocumentChunk[] {
        const chunks: DocumentChunk[] = [];
        const content = file.content;
        const chunkSize = this.config.chunkSize!;
        const overlap = this.config.chunkOverlap!;
