...
import { ProjectAnalysisResult, FileInfo } from './ProjectAnalyzer';
import { DeepWikiConfig } from '../config/ConfigManager';
import { VectorRetriever, EmbedderFactory } from './VectorRetriever';
import { Document } from '../types';
import { promises as fsPromises } from 'fs';
import { DocumentProcessor } from './DocumentProcessor';
import { RAGProcessor } from './RAGProcessor';
import { LangChainModelManager, ConversationTurn, DeepResearchConfig } from './LangChainModelManager';
...
export class WikiGenerator {
    ...
    private langChainManager: LangChainModelManager;
    ...
    constructor(config: DeepWikiConfig) {
        ...
        this.langChainManager = new LangChainModelManager(config);
        console.log('LangChainModelManager initialized successfully');
        
        // 初始化 RAGProcessor
        console.log('Initializing RAGProcessor...');
        this.ragProcessor = new RAGProcessor(this.vectorRetriever, config);
    ...
    async generateDeepResearchContent(
        ...
        iterationType: 'first' | 'intermediate' | 'final' = 'first',
        ...
        try {
            ...
            const result = await this.langChainManager.generateDeepResearchContent(
                query,
                deepResearchConfig,
                conversationHistory,
                '', // contextText
     ...
     * 获取与查询相关的文档
     */
    private async getRelevantDocuments(query: string, analysisResult: ProjectAnalysisResult): Promise<any[]> {
        try {
            // 这里可以使用向量检索来获取相关文档
            // 目前先返回项目的主要文件信息
            const documents: any[] = [];
     ...
     * 保存向量化缓存
     */
    private async saveVectorCache(documents: Document[]): Promise<void> {
        try {
            ...
            await fsPromises.writeFile(vectorCacheFile, JSON.stringify(cacheData, null, 2), 'utf8');
     ...
     * 加载向量化缓存
     */
    async loadVectorCache(): Promise<boolean> {
        try {
            const cacheDir = path.join(this.projectPath, '.deepwiki', 'cache');
            const vectorCacheFile = path.join(cacheDir, 'vectors.json');
            
            const cacheData = JSON.parse(await fsPromises.readFile(vectorCacheFile, 'utf8'));
     ...
     * 重建向量索引
     */
    async rebuildVectorIndex(analysisResult: ProjectAnalysisResult): Promise<void> {
        ...
        try {
            const cacheDir = path.join(this.projectPath, '.deepwiki', 'cache');
            const vectorCacheFile = path.join(cacheDir, 'vectors.json');
            await fsPromises.unlink(vectorCacheFile);
        } catch (error) {
            // 忽略删除失败的错误
        }
     ...
     * 生成项目概览
     */
    private async generateOverview(analysisResult: ProjectAnalysisResult): Promise<string> {
     ...
     * 生成架构说明
     */
    private async generateArchitecture(analysisResult: ProjectAnalysisResult): Promise<string> {
     ...
     * 生成数据流描述
     */
    private generateDataFlowDescription(analysisResult: ProjectAnalysisResult): string {
        const projectType = this.detectProjectType(analysisResult);
        const mainDirs = this.getMainDirectories(analysisResult);
        ...
        if (hasReactFiles || hasVueFiles || hasNextJs || hasNuxtJs) {
            ...
            // 根据主要目录添加节点
            const importantDirs = mainDirs.filter(dir => 
                !['node_modules', '.git', 'dist', 'build', '.vscode'].includes(dir.name)
            ).slice(0, 4);
     ...
     * 生成文件结构
     */
    private generateFileStructure(analysisResult: ProjectAnalysisResult): string {
        let structure = `# 📁 文件结构\n\n`;
     ...
     * 渲染树结构
     */
    private renderTree(node: Record<string, any>, prefix: string, isLast: boolean): string {
        let result = '';
     ...
     * 渲染结构树
     */
    private renderStructureTree(node: any, depth: number): string {
    ...
    private async generateCodeAnalysis(
        analysisResult: ProjectAnalysisResult,
        progressCallback?: (progress: number, message: string) => void
    ): Promise<CodeAnalysisSection[]> {
        const sections: CodeAnalysisSection[] = [];
     ...
     * 生成代码章节
     */
    private async generateCodeSection(fileType: string, files: FileInfo[], progressCallback?: (progress: number, message: string) => void): Promise<CodeAnalysisSection | null> {
        ...
        const section: CodeAnalysisSection = {
     ...
     * 生成文件类型分析
     */
    private generateTypeAnalysis(type: string, files: FileInfo[]): string {
     ...
     * 获取类型特定分析
     */
    private getTypeSpecificAnalysis(type: string, files: FileInfo[]): string {
        switch (type) {
            case 'javascript':
            case 'typescript':
     ...
     */
    private analyzeJSFiles(files: FileInfo[]): string {
     ...
     * 分析 Python 文件
     */
    private analyzePythonFiles(files: FileInfo[]): string {
     ...
     * 分析 CSS 文件
     */
    private analyzeCSSFiles(files: FileInfo[]): string {
     ...
     * 分析 JSON 文件
     */
    private analyzeJSONFiles(files: FileInfo[]): string {
     ...
     * 使用AI创建代码块（深度分析）
     */
    private async createCodeBlockWithAI(file: FileInfo): Promise<CodeBlock | null> {
     ...
     */
    private createCodeBlock(file: FileInfo): CodeBlock | null {
        const language = this.getLanguageFromExtension(file.extension);
     ...
     * 生成依赖说明
     */
    private async generateDependencies(analysisResult: ProjectAnalysisResult): Promise<string> {
        let dependencies = `## Dependencies\n\n`;
        
        // 查找包管理文件
        const packageFiles = analysisResult.files.filter(f => 
            ['package.json', 'requirements.txt', 'Gemfile', 'go.mod', 'Cargo.toml'].includes(path.basename(f.path))
        );
     ...
     * 生成设置说明
     */
    private async generateSetup(analysisResult: ProjectAnalysisResult): Promise<string> {
     ...
     * 生成使用说明
     */
    private async generateUsage(analysisResult: ProjectAnalysisResult): Promise<string> {
        let usage = `## Usage\n\n`;
        
        // 查找 README 文件
        const readmeFile = analysisResult.files.find(f => 
            path.basename(f.path).toLowerCase().startsWith('readme')
        );
    ...
    private analyzeArchitecturePatterns(analysisResult: ProjectAnalysisResult): string[] {
        ...
        // 检查RESTful API
        const hasApiRoutes = dirNames.includes('api') || dirNames.includes('routes') || files.some(f => 
        ...
        // 检查事件驱动架构
        const hasEventHandlers = files.some(f => f.content && (
        ...
        const hasDataBinding = files.some(f => f.content && (
            f.content.includes('v-model') || 
    ...
    private getMainDirectories(analysisResult: ProjectAnalysisResult): Array<{name: string, description: string}> {
        const mainDirs = [];
    ...
    private selectImportantFiles(files: FileInfo[], maxCount: number): FileInfo[] {
        // 按文件大小和重要性排序
        return files
            .sort((a, b) => {
                // 优先选择主要文件
    ...
    private detectProjectType(analysisResult: ProjectAnalysisResult): string {
        const files = analysisResult.files;
        ...
        const dominantType = fileCounts.reduce((max, current) => 
     ...
     * 生成技术栈信息
     */
    private generateTechStack(analysisResult: ProjectAnalysisResult): string {
     ...
     * 调用 AI 生成内容
     */
    private async generateAIContent(prompt: string, context?: string): Promise<string> {
        if (!this.config.provider || !this.config.apiKey) {
     ...
     * 构建文件分析专用的AI提示词
     */
    private buildFileAnalysisPrompt(filePath: string, language: string, code: string): string {
     ...
     * 构建 AI 提示词
     */
    private buildAIPrompt(prompt: string, context?: string): string {
     ...
     */
    private async performVectorization(
        analysisResult: ProjectAnalysisResult, 
        wikiData: WikiData, 
        progressCallback?: (progress: number, message: string) => void
    ): Promise<void> {
        ...
        try {
            ...
            const projectDocuments = await this.documentProcessor.processDirectory(
                this.projectPath,
    ...
    async queryWithRAG(
        ...
        answer: string;
        sources: Array<{ document: Document; similarity: number; }>;
        confidence: number;
    }> {
