...
export interface DeepWikiConfig {
    ...
    apiKey: string;
    baseUrl?: string;
    workspaceId?: string; // 用于 Qwen/Dashscope 工作空间 ID
    excludedDirs: string[];
...
export class ConfigManager {
    ...
    private configSection = 'deepwiki';
     ...
     * 测试 API 连接
     */
    async testConnection(config?: DeepWikiConfig): Promise<{ success: boolean; message: string }> {
     ...
     * 从 webview 更新配置
     */
    private async updateConfigFromWebview(data: any): Promise<void> {
...
<body>
     ...
     * 更新向量模型配置
     */
    async updateEmbedderConfig(embedderConfig: EmbedderConfig): Promise<void> {
     ...
     * 生成向量模型配置文件
     */
    async generateEmbedderConfigFile(projectPath: string): Promise<void> {
        ...
        await fsPromises.writeFile(
            configPath, 
            JSON.stringify({ embedder: embedderConfig }, null, 2), 
            'utf8'
        );
    }
}